    ...
    public class UserController : IUserController
    {
        ...
        private User DBToLogic(UserDB userDB)
        {
            ...
            if (userDB != null)
            {
                ...
                foreach (var userSkill in userDB.UserSkills)
                {
                    user.Skills.Add(skillController.DBToLogic(userSkill.Skill));
                }
                ...
                foreach (var userFile in userDB.UserFiles)
                {
                    if (userFile.File.Deleted != null)
                    {
                        continue;
                    }

                    var fileLogic = new File();
                    fileLogic.Name = userFile.File.Name;
                    fileLogic.ID = userFile.File.Id;
